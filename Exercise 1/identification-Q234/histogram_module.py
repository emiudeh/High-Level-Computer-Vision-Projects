...
def rgb_hist(img_color, num_bins):
    ...
    img_color = np.around((img_color/255) * (num_bins - 1)).astype(int)
    ...
    for i in range(img_color.shape[0]):
        for j in range(img_color.shape[1]):
            ...
            # your code
            hists[img_color[i, j, 0], img_color[i, j, 1], img_color[i, j, 2]] += 1
...
def rg_hist(img_color, num_bins):
    ...
    # your code here
    img_color = np.around((img_color/np.amax(img_color)) * (num_bins - 1)).astype(int)

    # execute the loop for each pixel in the image 
    for i in range(img_color.shape[0]):
        for j in range(img_color.shape[1]):
            # increment a histogram bin which corresponds to the value of pixel i,j; h(R,G)
            hists[img_color[i, j, 0], img_color[i, j, 1]] += 1
